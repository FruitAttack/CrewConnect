...
const MyCrew = () => {
    ...
    try {
      ...
      memberMap.forEach(({ user }, userId) => {
        ...
        if (approval) {
          switch (approval.status) {
            ...
            case "pending_changes":
              timecardStatus = "pending_changes";
              break;
            default:
        ...
        const time_off_requests = rawTimeOff.map((r) => ({
          ...
          reviewed_date: r.reviewed_at
            ? r.reviewed_at.split("T")[0]
            : undefined,
  ...
  const handleApprove = async (member, notes = null) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        Alert.alert(
          "Approved ✓",
          `${member.name}'s timecard has been approved.`,
        );
      } else {
  ...
  const handleDenyTimeOff = async () => {
    ...
    try {
      const res = await denyTimeOff(
        token,
        selectedTimeOffRequest.id,
        denyReason.trim() || "Denied by supervisor",
      );
      if (res.success) {
        setCrewMembers((prev) =>
          prev.map((m) => ({
  ...
  const getPendingTimeOffCount = () =>
    crewMembers.reduce(
      (n, m) =>
        n +
        (m.time_off_requests?.filter((r) => r.status === "pending").length ||
          0),
      0,
    );
      ...
      <View style={styles.tabsContainer}>
        ...
        >
          ...
          <Text
            style={[
              styles.tabText,
              selectedTab === "timecards" && styles.tabTextActive,
            ]}
          >
            Timecards
          </Text>
        ...
        >
          ...
          <Text
            style={[
              styles.tabText,
              selectedTab === "timeoff" && styles.tabTextActive,
            ]}
          >
            Time Off
          </Text>
      ...
      {selectedTab === "timecards" && (
        <View style={styles.weekNav}>
          ...
          <Pressable
            ...
            style={[
              styles.weekNavArrow,
              isLatestWeek && styles.weekNavArrowDisabled,
            ]}
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtext}>
            {searchQuery
              ...
              : "You haven't been assigned as a foreman yet"}
          </Text>
        </View>
      ) : (
        <ScrollView
          ...
          {selectedTab === "timecards" ? (
                  ...
                  {actionableTimecards.map((member) => (
                    ...
                    >
                      ...
                      <View style={styles.actionInfo}>
                        ...
                        <View
                          style={[
                            styles.statusBadge,
                            {
                              backgroundColor: `${getStatusColor(member.timecard.status)}20`,
                            },
                          ]}
                        >
                          <View
                          ...
                          <Text
                            style={[
                              ...
                              { color: getStatusColor(member.timecard.status) },
                            ]}
                          >
                      ...
                      <View style={styles.actionHours}>
                        <Text style={styles.actionHoursValue}>
                    ...
                    <View style={styles.infoSection}>
                      ...
                      <View
                        style={[
                          styles.statusBadge,
                          {
                            backgroundColor: `${getStatusColor(member.timecard.status)}18`,
                          },
                        ]}
                      >
                        <View
                      ...
                      <View style={styles.dateRow}>
                        ...
                        <Text style={styles.dateText}>
                          {formatDateRange(
                            member.current_period.start_date,
                            member.current_period.end_date,
                          )}
                        </Text>
                      </View>
                    </View>
            ...
            <>
              {filteredCrewMembers.map((member) => {
                ...
                return (
                  <View
                    key={`timeoff-${member.id}`}
                    style={styles.timeOffEmployeeCard}
                  >
                    <View style={styles.timeOffEmployeeHeader}>
                      <View
                        style={[
                          styles.timeOffHeaderAvatar,
                          { backgroundColor: member.color },
                        ]}
                      >
                        <Text style={styles.timeOffHeaderAvatarText}>
                          {member.initials}
                        </Text>
                      </View>
                      <View style={styles.timeOffHeaderInfo}>
                        <Text style={styles.timeOffHeaderName}>
                          {member.name}
                        </Text>
                      ...
                      <View key={req.id}>
                        ...
                        >
                          <View style={styles.timeOffRequestLeft}>
                            <View
                              style={[
                                styles.timeOffTypeBadge,
                                {
                                  backgroundColor: `${getTimeOffTypeColor(req.type)}15`,
                                },
                              ]}
                            >
                              <Text
                                style={[
                                  styles.timeOffTypeText,
                                  { color: getTimeOffTypeColor(req.type) },
                                ]}
                              >
                                {req.type}
                              </Text>
                          ...
                          <View style={styles.timeOffRequestRight}>
                            <View
                              style={[
                                styles.timeOffStatusBadge,
                                {
                                  backgroundColor: `${getTimeOffStatusColor(req.status)}20`,
                                },
                              ]}
                            >
                              <View
                              ...
                              <Text
                                style={[
                                  ...
                                  { color: getTimeOffStatusColor(req.status) },
                                ]}
                              >
              ...
              {filteredCrewMembers.every(
                (m) => !m.time_off_requests?.length,
              ) && (
                <View style={styles.emptyContainer}>
                ...
                <>
                  ...
                  <View style={styles.modalEmployeeSection}>
                    <View
                      style={[
                        styles.modalAvatar,
                        { backgroundColor: selectedCrewMember.color },
                      ]}
                    >
                      <Text style={styles.modalAvatarText}>
                        {selectedCrewMember.initials}
                      </Text>
                    </View>
                    ...
                    <Text style={styles.modalPeriodDate}>
                      {formatDateRange(
                        selectedCrewMember.timecard.start_date,
                        selectedCrewMember.timecard.end_date,
                      )}
                    </Text>
                    <View
                      style={[
                        styles.statusBadge,
                        {
                          backgroundColor: `${getStatusColor(selectedCrewMember.timecard.status)}20`,
                          marginTop: 8,
                        },
                      ]}
                    >
                      <View
                        style={[
                          ...
                          {
                            backgroundColor: getStatusColor(
                              selectedCrewMember.timecard.status,
                            ),
                          },
                        ]}
                      />
                  ...
                  <View style={styles.dailyBreakdown}>
                    ...
                    <View style={styles.dailyTable}>
                      ...
                      {selectedCrewMember.timecard.daily_hours?.map(
                        (day, idx) => (
                          <View key={idx} style={styles.tableRow}>
                            <Text style={[styles.tableCellDate, { flex: 1 }]}>
                              {formatDate(day.date)}
                            </Text>
                            <Text style={[styles.tableCellDay, { flex: 1 }]}>
                              {day.day_name}
                            </Text>
                            <Text
                              style={[
                  ...
                  <View style={styles.summarySection}>
                    {[
                      {
                        label: "REGULAR",
                        val: selectedCrewMember.timecard.regular_hours,
                      },
                      {
                        label: "OVERTIME",
                        val: selectedCrewMember.timecard.overtime_hours,
                      },
                    ].map(({ label, val }) => (
                    ...
                    {[
                      {
                        ...
                        signed: selectedCrewMember.timecard.employee_signed,
                      },
                      {
                        label: "Supervisor Signature",
                        signed: selectedCrewMember.timecard.supervisor_signed,
                      },
                    ].map(({ label, signed }) => (
                ...
                <>
                  <View style={styles.modalEmployeeSection}>
                    ...
                    >
                      ...
                      >
                        {selectedTimeOffRequest.status.charAt(0).toUpperCase() +
                          selectedTimeOffRequest.status.slice(1)}
                      </Text>
                    </View>
                  </View>
                    ...
                    {[
                      {
                        ...
                        value: formatDateRange(
                          selectedTimeOffRequest.start_date,
                      ...
                      },
                      {
                        icon: "time-outline",
                        label: "Duration",
                        value: `${selectedTimeOffRequest.days} ${selectedTimeOffRequest.days === 1 ? "day" : "days"}`,
                      },
                      {
                        icon: "document-text-outline",
                        label: "Submitted",
                        value: formatDate(
                          selectedTimeOffRequest.submitted_date,
                        ),
                      },
                  ...
                  {selectedTimeOffRequest.reason ? (
                    <View style={styles.timeOffReasonSection}>
                      ...
                      <View style={styles.timeOffReasonBox}>
                        <Text style={styles.timeOffReasonText}>
                          {selectedTimeOffRequest.reason}
                        </Text>
                      </View>
                    </View>
                  ...
                  {selectedTimeOffRequest.status === "pending" && (
                    <View style={styles.modalActions}>
                      <Pressable
                        ...
                        onPress={() =>
                          handleApproveTimeOff(selectedTimeOffRequest)
                        }
                      >
                        <Ionicons
                          name="checkmark-circle"
